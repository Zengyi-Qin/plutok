...
if __name__ == "__main__":
    ...
    device = "cuda"
    audio_path = "resource/example_reference_16khz.mp3"  # "tellitall_00_stenhouse_64kb_cut.mp3"
    output_path = "./output.mp3"
    plutok_tokenizer = Plutok(ov_ckpt, centroids, tokenizer_file, device)
    wav, _ = librosa.load(audio_path, sr=22050)

    #feat = np.load(open('sampled_feat.npy', 'rb'))[:1]
    #wav_dec = plutok_tokenizer.reconstruct(wav, feat).squeeze()
